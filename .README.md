...
With the help of GitHub Actions, `update.sh` makes possible the following badges, and more! To make a badge, you can modify one below or generate one with something like [shields.io/json](https://shields.io/badges/dynamic-json-badge) and [these parameters](#url). If we don't yet follow a user or org, you can either:

* open an issue or
* add them on a new line in `pkg.txt` on your own fork [here](https://github.com/ipitio/<GITHUB_REPO>/edit/master/pkg.txt) and make a pull request.
...
```markdown
https://raw.githubusercontent.com/<GITHUB_OWNER>/<GITHUB_REPO>/<GITHUB_BRANCH>/index.json
```
