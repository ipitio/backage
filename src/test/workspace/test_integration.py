...
def test_sparse_repository_retains_site_content_across_owner_waves(
    ...
    asset = path / ".bkg-site" / "assets" / "app.js"
    asset.parent.mkdir(parents=True)
    shell.write_text("prior shell\n", encoding="utf-8")
    ...
    asset.write_text("next asset\n", encoding="utf-8")

    repository.materialize_sparse_paths(("beta",), replace=True)

    assert shell.read_text(encoding="utf-8") == "next shell\n"
    assert asset.read_text(encoding="utf-8") == "next asset\n"
    assert _git(path, "diff", "--cached", "--name-only").stdout.splitlines() == [
        ".bkg-site/assets/app.js"
    ]
    assert _git(path, "diff", "--name-only").stdout.splitlines() == ["index.html"]
