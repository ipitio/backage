...
EOF
	...
	(
		...
		export -n GITHUB_TOKEN BKG_HTTP_TOTAL_TIMEOUT BKG_GITHUB_REST_RESERVE BKG_MAX_VERSION_PAGES BKG_TAG_CACHE_PAGES BKG_APPEND_TAGGED_VERSIONS_LIMIT BKG_PARALLEL_ASYNC_MAX_JOBS BKG_OWNER_UPDATE_STOP_GRACE
		BKG_PYTHON="$fake_python"
		bkg_python github rest users/ipitio
	) >"$output_file"

	assert_contains "$output_file" "local-token|45|17|7|2|11|5|12.5|-m bkg_py github rest users/ipitio"
}
