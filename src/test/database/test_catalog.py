...
def test_catalog_seed_preserves_tree_paths_across_history_pruning(
    ...
    repository.write_package(_record(resolved, "2026-06-01"))

    status = repository.initialize_package_catalog(
        (resolved, tree_only),
        "a" * 40,
        TODAY,
    )
...
def test_catalog_seed_rolls_back_before_readiness_on_failure(tmp_path: Path) -> None:
    ...
    repository.write_package(_record(retained))
    ...
    assert repository.package_catalog_status() is None
...
def test_catalog_resynchronizes_to_a_new_index_revision(tmp_path: Path) -> None:
    ...
    repository.initialize_package_catalog((old, retained), "a" * 40, TODAY)
...
def test_complete_scan_enriches_observed_and_retires_tree_only_paths(
    ...
    assert repository.package_inventory() == PackageInventory(1, 1, 1)
    status = repository.package_catalog_status()
...
def test_catalog_tracks_package_and_owner_retirements(tmp_path: Path) -> None:
    ...
    repository.initialize_package_catalog((first, second), "d" * 40, TODAY)
    first_record = _record(first)
    second_record = _record(second)
    repository.write_package(first_record)
    repository.write_package(second_record)

    repository.retire_package(first_record.package_ref)

    assert repository.package_inventory() == PackageInventory(1, 1, 1)

    repository.retire_owner(second.owner)

    assert repository.package_inventory() == PackageInventory(0, 0, 0)
