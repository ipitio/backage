...
docker run -v $PWD/<GITHUB_REPO>:/app --env-file <(env | grep GITHUB) ghcr.io/<GITHUB_OWNER>/<GITHUB_REPO>:<GITHUB_BRANCH> bash src/test/update.sh
```
