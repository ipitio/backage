...
refresh_package() {
    ...
    echo "{
        ...
        \"version\":
    [" >"$json_file"
    run_parallel refresh_version "$(sqlite3 "$BKG_INDEX_DB" "select id, name, size, downloads, downloads_month, downloads_week, downloads_day, date, tags from '$table_version_name' where date='$max_date' group by id;")" || return $?
